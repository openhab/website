...
With openHAB 3 we reach a new level of both functionality as well user-friendliness. Evolving a product with such a large userbase is always a difficult challenge. On the one hand, users always wish new features to be integrated and contributors have ideas and want to see their implementations being accepted and integrated. There is hence the continuous risk of a massive feature creep. On the other hand, newbies want a clear path and best practices on how to use the product and having too many different ways and options can easily confuse them as it leads to a steep learning curve.

The balance found for openHAB 3 is really a sweet spot: The underlying framework has been modernized without hardly any impact, a lot of complexity has been thrown overboard without dropping features that our users rely on and many things have been streamlined, which culminates in our completely new main user interfaces.
...
Version 2 of openHAB had to do compromises between the "old" purely textual openHAB 1 world and the new UI-enabling features that were introduced with Eclipse SmartHome. The way one had to work with bindings and rules very much differed from each other and these two world existed more or less side-by-side, held together through the 1.x compatibility layer.

A huge step of simplification in openHAB 3 was therefore the removal of the 1.x compatibility layer. All bindings now work exactly the same way and the user decides on whether he prefers configuration to be done textually or through the UI. To make sure that nobody is left behind by this move, [many 1.x bindings have been reimplemented](https://github.com/openhab/openhab-addons/issues/6179#migrated) for openHAB 3 and for the rest, it is possible to keep an old 2.x instance running while integrating it into a new 3.0 setup - see the [outposts section](#Outposts) for details.

Also, the two separate rule engine implementations have been merged into one, so that there is now a single concept for automation rules. As a consequence, textual rules written in openHAB's own Rule DSL also turn up within the user interface. 
...
<p align="center"><video src="/uploads/evolution.m4v"/></p>
...
Such a setup then consists out of a central openHAB instance and several openHAB outposts. These can make sense, if certain devices need physical vicinity, e.g. because they need to be connected through a serial interface or because their wireless reach is not sufficient to reach the central instance like it can be the case for Bluetooth sensors.
...
The new user interface also features a brand new way of designing your automation scripts using [Blockly](https://developers.google.com/blockly), a tool to make your program visually by assembling blocks.

<p align="center"><img src="/uploads/2020-12_blockly.jpg"/></p>

### New Pages Concept

Last but not least, the UI features new ways of interacting with your system: You can design several types of pages - a new concept - like maps or floor plans, with extensive features to help navigate your system. 
