...
1.  Through [Paper UI](/addons/uis/paper/readme.html).
...
Graphical UIs will display the label text when the Item is included, e.g. in [Basic UI](/addons/uis/basic/readme.html) in a [Sitemap](/configuration/sitemaps.html) definition.
...
*N.B.*  Many openHAB users find that it can be very useful to use [Persistence](/addons/persistence.html) and [System started](/configuration/rules-dsl.html#system-based-triggers) rules so that their systems behaves in a predictable way after an openHAB restart.
...
openHAB provides a set of [classic icons](/addons/iconsets/classic/readme.html) by default.
...
These relationships can be exploited in [Sitemaps](/configuration/sitemaps.html) or in [automation rules](/configuration/rules-dsl.html) to navigate through the hierarchically organized Items or to perform computations and updates on subsets of similar Items.
...
See the [Hue Emulation](/addons/ios/hueemulation/readme.html) or [HomeKit Add-on](/addons/ios/homekit/readme.html) documentation for more details.

{: #binding}
### Binding Configuration

One of the greatest strengths of an openHAB automation system is the sheer number of devices you can interact with.
See "[currently available Bindings](/addons/bindings.html)" for a list of available Bindings.
...
The information required for each binding is specified in the configuration information provided for each of the available [Bindings](/addons/bindings.html).
...
openHAB2 introduces the concept of [Things and Channels](/concepts/things.html).
...
See the [Bindings](/addons/bindings.html) configuration section for more information on how to discover or manually define Things for a given Binding.
...
The second example shows a common use case for the [expire Binding](/addons/bindings/expire1/readme.html)
