...
The [demo setup](/tutorials/demo.html) already comes with a demo file called `demo.rules`, which has a couple of examples that can be a good starting point.
...
The [Eclipse SmartHome Designer](/configuration/editors.html#esh-designer) offers support for rules building.
...
You can find all the possible values for status from [Thing Status](/concepts/things.html).
And refer to [Thing Status Action](/addons/actions.html) to find how to get thing status in the script.
...
- Use the [Eclipse SmartHome Designer](/installation/designer.html) and the `<ctrl><space>` key combo to list all the available methods
...
*Reminder: For a complete and up-to-date list of what item types are currently allowed in openHAB and the command types each item can accept refer to the section on [items in the openHAB documentation](/concepts/items.html).*
...
openHAB [Transformation services](/addons/transformations.html) may also be used in rules to transform/translate/convert data.
...
For all available Transformation services please refer to the list of [Transformation Add-ons](/addons/transformations.html).
