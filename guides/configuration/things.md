...
Things are connected to openHAB through [bindings](/addons/bindings.html).
...
5. [Add Items](/configuration/items.html) and link them to the Thing's Channels
6. At this point Items can be used to control the Thing or consume its information in e.g. [Sitemaps](/configuration/sitemaps.html) or [Rules](/configuration/rules-dsl.html)
...
For some bindings (such as the [YahooWeater](/addons/bindings/yahooweather/readme.html) binding), manual Thing definitions are required.
Other bindings (such as the [ZWave](/addons/bindings/zwave/readme.html) binding) currently prefer or require the discovery method.
...
Newly discovered Things can be added to the system from the inbox using the [Paper UI](/configuration/paperui.html).
...
See the [configuration tutorial](/tutorials/beginner/configuration.html) for a step-by-step guide on how to configure Things using discovery.
...
- the binding ID is "network" (using the [Network Binding](/addons/bindings/network/readme.html))
...
Please check each individual binding's [documentation](/addons/bindings.html) for details on what and how to define Things using the `*.things` configuration text files.
